...
def calculate_auto_step_result(file_name, axs):
    ...
    try:
        ...
        # Проверяем, есть ли достаточно столбцов
        if tbl.shape[1] < 2:
            print(f"В файле {file_name} недостаточно столбцов. Пропускаем.")
        ...
        n_points = n_cols // 3
        if n_points < 4:
            print(f"Недостаточно данных для 4 лап в файле {file_name}. Пропускаем.")
