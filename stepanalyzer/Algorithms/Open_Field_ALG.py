...
#     stat_tabl = np.full((Na, len(metrics) * 6), np.nan)
#
...
def calculate_step_result(cofig, axs, check_auto):
    metrics = ['Median stride time', 'Mean stride time', 'Kvar stride time', 'Frac stride time',
               'Median stride intl', 'Mean stride intl', 'Kvar stride intl', 'Frac stride intl']
    ...
    try:
        try:
            with open(cofig.name_csv_file, 'r') as f:
        ...
        # Проверяем, есть ли достаточно столбцов
        if tbl.shape[1] < 2:
            print(f"В файле {cofig.name_csv_file} недостаточно столбцов. Пропускаем.")
        ...
        n_points = n_cols // 3
        if n_points < 4:
            print(f"Недостаточно данных для 4 лап в файле {cofig.name_csv_file}. Пропускаем.")
        ...
        Np = 6
        # Индексы для лап (0-based)
        Npts = [1, 2, 4, 5, 7, 8, 10, 11]  # Левая передняя, правая передняя, левая задняя, правая задняя
        caps = ['Left front paw', 'Right front paw', 'Left hind paw', 'Left knee', 'Right hind paw', 'Right knee']
        TypeColor = np.array([[0, 0.4470, 0.7410], [0.3010, 0.7450, 0.9330],
                              [0.8500, 0.3250, 0.0980], [1, 0, 0], [1, 0, 0], [1, 0, 0]])
        X_raw = [None] * Np
        ...
        if check_auto:
            ...
            stat_tabl = []
            stepCycleDetector = StepCycleDetector(config=cofig)
            for i in range(0, 6):
        ...
        print(f"Ошибка при обработке файла {cofig.name_csv_file}: {e}")
...
def manual_calculate_result(self, data_init):
    ...
    current_state = 'looking_for_min'  # или 'looking_for_max'
    current_cycle = []

    for i in range(len(peaks_list)):
        peak = peaks_list[i]

        if current_state == 'looking_for_min' and peak in peaks_min:
            # Нашли стартовый минимум
            current_cycle.append(peak)
            current_state = 'looking_for_max'

        elif current_state == 'looking_for_max' and peak in peaks_max:
            # Проверяем валидность максимума
            if len(current_cycle) == 1:  # есть минимум
                min_idx = current_cycle[0]
                max_idx = peak

                # Проверяем амплитуду (разницу значений, а не индексов!)
                amplitude = abs(data_init[max_idx] - data_init[min_idx])

                if (self.speed_change_min.value() < amplitude < self.speed_change_max.value() and
                        self.travel_time_min.value() < (max_idx - min_idx) < self.travel_time_max.value()):

                    current_cycle.append(peak)
                    current_state = 'looking_for_end_min'
                else:
                    # Невалидный максимум - сбрасываем
                    current_cycle = []
                    current_state = 'looking_for_min'

        elif current_state == 'looking_for_end_min' and peak in peaks_min:
            # Проверяем завершающий минимум
            if len(current_cycle) == 2:  # есть минимум и максимум
                start_min_idx = current_cycle[0]
                max_idx = current_cycle[1]
                end_min_idx = peak

                # Проверяем время цикла
                cycle_time = end_min_idx - start_min_idx

                if self.travel_time_min.value() < cycle_time < self.travel_time_max.value():
                    # Валидный цикл
                    current_cycle.append(peak)
                    temp_peaks_result.extend(current_cycle)
    ...
    if len(temp_peaks_result) >= 3:
        for i in range(0, len(temp_peaks_result) - 2, 3):
            if i + 2 < len(temp_peaks_result):  # Проверяем наличие полного цикла
                start_min = temp_peaks_result[i]
                max_point = temp_peaks_result[i + 1]
                end_min = temp_peaks_result[i + 2]

                # Заполняем область между минимумами
                for j in range(start_min, min(end_min + 1, len(data_init))):
                    temp_result[j] = data_init[max_point] + 100
    return temp_result
...
def analyze_step_parameters(step_data):
    ...
    stat_tbl.append(np.std(steps) / np.mean(steps) if np.mean(steps) != 0 else 0.0)
    stat_tbl.append(np.sum(steps) / len(steps) if len(steps) > 0 else 0.0)

    stat_tbl.append(median(none_steps))
    stat_tbl.append(mean(none_steps))
    stat_tbl.append(np.std(none_steps) / np.mean(none_steps) if np.mean(none_steps) != 0 else 0.0)
    stat_tbl.append(np.sum(none_steps) / len(none_steps) if len(none_steps) > 0 else 0.0)
