...
#
#             plt.suptitle(f'Animal: {os.path.basename(animals[animal])}')
...
def calculate_auto_step_result(self, axs, check_auto):
    metrics = ['Median stride time', 'Mean stride time', 'Kvar stride time', 'Frac stride time',
               'Median stride intl', 'Mean stride intl', 'Kvar stride intl', 'Frac stride intl']
    ...
    try:
        ...
        if check_auto:
            ...
            axs[0].plot(manual_calculate_result(self, X_raw[0]))
            axs[1].plot(manual_calculate_result(self, X_raw[1]))
            axs[2].plot(manual_calculate_result(self, X_raw[2]))
            axs[3].plot(manual_calculate_result(self, X_raw[3]))
...
def manual_calculate_result(self, data_init):
    ...
    for i in range(0, len(temp_array)):
        if not start_step and not prev_min and temp_array[i] in peaks_max:
            ...
            if temp_array[i] in peaks_min:
                if abs(data_init[temp_array[i]] - data_init[result[-1]]) > self.spinbox_angle.value():
                    result.append(temp_array[i])
                    prev_min = True
                else:
                    result.pop()
    ...
    for i in range(0, len(result) - 2, 3):
        for j in range(result[i], result[i + 2] + 1):
            temp_result[j] = self.data_init[result[i]]
    return temp_result
